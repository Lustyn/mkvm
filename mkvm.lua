...
local default_mounts = {"rw:/:/vms/%NAME%","r:/rom:/rom","r:/bios.lua:/vms/%NAME%/bios.lua"}
local default_bios_path = "/vms/%NAME%/bios.lua"
...
if ebios_disabled then
  bios_cmd = {"wget","https://raw.githubusercontent.com/dan200/ComputerCraft/master/src/main/resources/assets/computercraft/lua/bios.lua",pargs["--boot"]}
else
  bios_cmd = {"pastebin","get","Jha0CZ8w",pargs["--boot"]}
end
...
if not ebios_disabled then
  ask_for_file("/vms/"..name.."/.settings", {resolve("/vms/"..name.."/.settings")}, {"pastebin", "get", "iThiduNQ", resolve("/vms/"..name.."/.settings")})
